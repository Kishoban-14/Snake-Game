...
Food::~Food()
{
 ...
 void Food::generateFood(objPos blockOff, int boardWidth, int boardHeight)
 {
    if(!isFoodGenerated)
    {
        ...
        randX = rand() % (boardWidth - 2) + 1; //Kishoban- Random X within bounds(excluding edges) (Attempt at fixing issue of food object being placed out of bounds)
        randY = rand() % (boardHeight - 2) + 1;//Kishoban- Random Y within bounds(excluding edges)
